...
The `runSPM` function supports two engines via the `engine` argument:

- `engine = "admb"` (default): calls the external ADMB `spm` executable and reads `spm_detail.csv`.
- `engine = "rtmb"`: runs the experimental RTMB path and writes `spm_detail_rtmb.csv`.

```{r atka,eval=TRUE}
#|
bdf <- runSPM(here::here("examples","atka"), run=FALSE, engine = "admb")
...
#| echo: TRUE
# Requires RTMB installed
bdf_rtmb <- runSPM(here::here("examples","atka"), run = TRUE, engine = "rtmb")
```
