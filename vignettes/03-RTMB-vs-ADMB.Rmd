---
title: "RTMB vs ADMB comparison (experimental)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{RTMB vs ADMB comparison (experimental)}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE)
```

This vignette compares the legacy ADMB workflow (`spm.tpl`) with the new
RTMB path. The RTMB path is **experimental** and currently supports
alternatives 1–5 only.

## Run both engines

```{r}
# Replace with your run directory that contains spm.dat
run_dir <- "examples/atka"

# ADMB run (requires spm binary)
# admb_res <- runSPM(run_dir, run = TRUE, engine = "admb")

# RTMB run
rtmb_res <- runSPM(run_dir, run = TRUE, engine = "rtmb")
```

## Compare outputs

```{r}
# If ADMB results are available, compare the distributions
# admb_res <- readr::read_csv(file.path(run_dir, "spm_detail.csv"))

if (exists("admb_res")) {
  summary(admb_res$SSB)
}
summary(rtmb_res$SSB)
```

```{r}
if (exists("admb_res")) {
  library(ggplot2)
  ggplot() +
    geom_density(data = admb_res, aes(x = SSB, color = "ADMB")) +
    geom_density(data = rtmb_res, aes(x = SSB, color = "RTMB")) +
    labs(x = "SSB", y = "Density", color = "Engine") +
    theme_minimal()
}
```

## Notes

- RTMB output is written to `spm_detail_rtmb.csv`.
- Differences are expected due to seed and the current experimental scope.
