...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
  ...
  pad_alt2_proj = new ofstream("alt2_proj.out");
  ...
  // Open species-specific level details
   for (int i=1;i<=nspp;i++)
   {
     ...
     *(ad_comm::global_datafile) >> FABC_Adj(i);             // 8
     cout <<"Adjusted ABC "<<FABC_Adj(i)<<endl;
...
void model_parameters::userfunction(void)
{
  ...
  ofstream& alt2_proj= *pad_alt2_proj;
...
void model_parameters::Run_Sim(void)
{
  ...
  ofstream& detail_out= *pad_detail_out;
  ofstream& prof_F= *pad_prof_F;
  detail_out<<"Alternative Spp Yr  ABC OFL Catch SSB F Tot_biom SPR_Implied Ntot SexRatio"<<endl;
    for (int ispp=1;ispp<=nspp;ispp++)
    {
...
void model_parameters::Mainloop(int& isim)
{
  ...
  ofstream& prof_F= *pad_prof_F;
   ...
   { // call and get TAC's fo this species in this year...
     ...
     for (int ispp=1;ispp<=nspp;ispp++) 
       detail_out<<alt<<" "<<spname(ispp)<<" "<<styr+ipro-1<<" "<<ABC(ispp)<<" "<<OFL(ispp)<<" "<<Actual_Catch(ispp)
                      <<" "<<SBsim(ispp,isim,ipro)<<" "<<Fsim(ispp,isim,ipro)  <<" "<<Bsim(ispp,isim,ipro)<<" "<<SPRsim(ispp,isim,ipro)
                      <<" "<<Nsim(ispp,isim,ipro) <<" "<<SRsim(ispp,isim,ipro)<< endl;
   } // Loop over projection years
}
...
void model_parameters::write_alt2(void)
{
  ofstream& means_out= *pad_means_out;
  ofstream& alt2_proj= *pad_alt2_proj;
  ofstream& percent_out= *pad_percent_out;
  ...
  ofstream& detail_out= *pad_detail_out;
  ofstream& prof_F= *pad_prof_F;
  alt2_proj << "Stock Year Catch ABC OFL SSB TotBiom"<<endl;
  for (int ispp=1;ispp<=nspp;ispp++)
  {
    ...
    for (i=2;i<=npro;i++)
    {
      alt2_proj << spname(ispp)<<" "<< i+styr-1 <<" "<<mean(ccc(i))<<" "<<(ABCs_by_yr(i,ispp))/nsims<<" "<<
...
void model_parameters::write_sim(const adstring& Title, d3_array& Outtmp)
{
  ...
  for (int ispp=1;ispp<=nspp;ispp++) for (int i=1;i<=npro;i++) for (k=1;k<=nsims;k++)
